...
namespace NatML.API.Services {
    ...
    public sealed class StorageService {
        ...
        /// <param name="dataLimit">Maximum size in bytes of feature stream for an upload URL to be created. Default to 4MB.</param>
        public async Task<string> Upload (
            string name,
            MemoryStream stream,
            UploadType type,
            string? mime = null,
            int dataLimit = 4 * 1024 * 1024
        ) {
            mime ??= @"application/octet-stream";
            // Data URL
            if (stream.Length < dataLimit) {
    ...
    internal sealed class CreateUploadURLInput {
        public string name;
        [JsonConverter(typeof(API.Graph.GraphEnum<UploadType>))]
