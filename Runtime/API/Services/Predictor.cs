...
namespace NatML.API.Services {
    ...
    public sealed class PredictorService {
        ...
        /// </summary>
        /// <param name="tag">Predictor tag.</param>
        public Task<string[]> Review (string tag) => client.Query<string[]>(
            @$"mutation ($input: ReviewPredictorInput!) {{
                reviewPredictor (input: $input)
            }}",
            @"reviewPredictor",
            new () {
                ["input"] = new ReviewPredictorInput {
                    tag = tag
                }
            }
        );
    ...
    internal sealed class UpdatePredictorInput {
        ...
        [JsonConverter(typeof(API.Graph.GraphEnum<AspectMode>))]
    ...
    internal sealed class ReviewPredictorInput : PredictorInput { }
