...
namespace NatSuite.ML.Internal {
    ...
    using ModelFlags = NatML.ModelFlags;
    ...
    public sealed class MLEdgeModel : MLModel {
        ...
        unsafe internal MLEdgeModel (string session, byte[] graph, ModelFlags flags) : base(session) {
            // Create
            fixed (void* buffer = graph)
                NatML.CreateModel(buffer, graph.Length, flags, out model);
