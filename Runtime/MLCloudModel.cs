...
namespace NatML {
    ...
    public sealed class MLCloudModel : MLModel {
        ...
        public async Task<MLFeatureCollection<MLCloudFeature>> Predict (params MLCloudFeature[] inputs) {
            ...
            // Predict
            var prediction = await client.EndpointPredictions.Create(tag, features, rawOutputs: true);
            // Check for errors
            if (!string.IsNullOrEmpty(prediction.error))
                throw new InvalidOperationException(prediction.error);
            // Download output features
            var results = await Task.WhenAll(prediction.results.Select(async feature => new MLCloudFeature(
