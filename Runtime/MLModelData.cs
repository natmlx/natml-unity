    ...
    public sealed class MLModelData : ScriptableObject {
        ...
        private static async Task<MLModelData> FromEmbed (string tag, string accessKey) {
            ...
            if (!embeddedData.TryGetValue(tag, out var embed))
                return null;
            // Check secret
            if (string.IsNullOrEmpty(embed.session.secret))
                return embed;
            // Get secret
            var CacheKey = $"{tag}.secret";
            embed.session.secret = PlayerPrefs.GetString(CacheKey, null);
            if (!string.IsNullOrEmpty(embed.session.secret))
                return embed;
            // Create secret
            var session = await CreateSession(tag, accessKey);
            PlayerPrefs.SetString(CacheKey, session.secret);
