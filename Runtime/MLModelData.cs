...
namespace NatSuite.ML {
    ...
    public sealed partial class MLModelData : ScriptableObject {
        ...
        public unsafe MLModel Deserialize () {
            ...
            var currentPlatform = platform == Platform.macOS ? Platform.iOS : platform;
            var sessionPlatform = session.platform == Platform.macOS ? Platform.iOS : session.platform;
            if (currentPlatform != sessionPlatform)
                throw new InvalidOperationException($"Cannot deserialize {session.platform} graph on {platform}");
            // Deserialize
