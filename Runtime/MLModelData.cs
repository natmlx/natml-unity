...
namespace NatML {
    ...
    public sealed class MLModelData : ScriptableObject {
        ...
        public unsafe MLModel Deserialize () {
            ...
            if (!string.IsNullOrEmpty(session.format) && session.format != FormatForPlatform(platform))
                throw new InvalidOperationException($"Cannot deserialize {session.format} graph on {platform}");
            // Deserialize
            switch (session.predictor.type) {
                case PredictorType.Edge:    return new MLEdgeModel(session.id, graph, session.flags);
                case PredictorType.Cloud:   return new MLCloudModel(session.id);
        ...
        internal static string FormatForPlatform (string platform) => platform switch {
        ...
        internal static async Task SaveToCache (MLModelData modelData, string cachePath) {
            // Check model data
            if (modelData == null)
                return;
            // Check path
