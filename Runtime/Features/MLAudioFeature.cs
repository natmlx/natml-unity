...
namespace NatML.Features {
    ...
    public sealed class MLAudioFeature : MLFeature, IMLEdgeFeature, IMLCloudFeature, IEnumerable<(MLAudioFeature feature, long timestamp)> {
        ...
        public MLAudioFeature (float[] sampleBuffer, int sampleRate, int channelCount) {
            this.type = new MLAudioType(sampleRate, channelCount, sampleBuffer.Length);
        ...
        public unsafe MLAudioFeature (float* sampleBuffer, int sampleRate, int channelCount, int sampleCount) {
            this.type = new MLAudioType(sampleRate, channelCount, sampleCount);
        ...
        public MLAudioFeature (string path) {
            this.type = MLAudioType.FromFile(path);
            this.path = path;
        ...
        public MLAudioFeature (MLCloudFeature feature) {
            // Check
            if (feature.type != DataType.Audio)
                throw new ArgumentException(@"Cloud feature is not an audio feature", nameof(feature));
            // Not implemented
            this.type = new MLAudioType(0, feature.shape[2], feature.shape[1] * feature.shape[2]);
