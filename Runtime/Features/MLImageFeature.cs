...
namespace NatML.Features {
    ...
    public unsafe class MLImageFeature : MLFeature, IMLEdgeFeature {
        ...
        public virtual unsafe void CopyTo (Texture2D destination) {
            // Check size
            ...
            // Copy data
            var dstBuffer = destination.GetRawTextureData<byte>().GetUnsafePtr();
            fixed (void* pixelData = this.pixelBuffer)
                fixed (void* colorData = this.colorBuffer) {
                    ...
                    UnsafeUtility.MemCpy(dstBuffer, data, width * height * 4);
                }
            destination.Apply();
        }
        #endregion
