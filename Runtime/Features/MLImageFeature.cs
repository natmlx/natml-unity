...
namespace NatML.Features {
    ...
    public sealed unsafe class MLImageFeature : MLFeature, IMLEdgeFeature, IMLCloudFeature {
        ...
        public MLImageFeature (Color32[] colorBuffer, int width, int height) : this(
            ToPixelBuffer(colorBuffer, width, height),
            width,
            height
        ) { }
        ...
        public void CopyTo (MLImageFeature destination, Rect rect, float rotation = 0f, Color32 background = default) => CopyTo(
            destination,
        ...
        /// <param name="rotation">Rectangle clockwise rotation in degrees.</param>
        /// <param name="background">Background color for unmapped pixels.</param>
        public unsafe void CopyTo (MLImageFeature destination, RectInt rect, float rotation = 0f, Color32 background = default) {
            // Check
        ...
        public unsafe void CopyTo (Texture2D destination, bool upload = true) {
            // Check size
        ...
        unsafe MLEdgeFeature IMLEdgeFeature.Create (MLFeatureType type) {
            fixed (void* data = this) {
                ...
                NatML.CreateFeature(
                    ...
                    height,
                    featureType.shape,
                    featureType.dataType.ToEdgeType(),
                    meanArr,
                    stdArr,
                    (int)aspectMode,
                    out var feature
        ...
        [Obsolete(@"Deprecated in NatML 1.1.0. Use `CopyTo` method instead.", false)]
        public MLImageFeature RegionOfInterest (Rect rect, float rotation = 0f, Color32 background = default) {
            var width = (int)(rect.width * this.width);
            var height = (int)(rect.height * this.height);
            var data = new byte[width * height * 4];
            var destination = new MLImageFeature(data, width, height);
            CopyTo(destination, rect, rotation, background);
            return destination;
        }

        [Obsolete(@"Deprecated in NatML 1.1.0. Use `CopyTo` method instead.", false)]
        public unsafe MLImageFeature RegionOfInterest (RectInt rect, float rotation = 0f, Color32 background = default) {
            var data = new byte[rect.width * rect.height * 4];
            var destination = new MLImageFeature(data, rect.width, rect.height);
            CopyTo(destination, rect, rotation, background);
            return destination;
        }
        #endregion
