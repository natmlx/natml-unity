...
namespace NatML.Features {
    ...
    public unsafe class MLImageFeature : MLFeature, IMLEdgeFeature {
        ...
        public MLImageFeature (Texture2D texture) {
            // Check texture
            if (!texture)
                throw new ArgumentException(@"Cannot create image feature because texture is `null`", nameof(texture));
            // Check readable
            if (!texture.isReadable)
                throw new ArgumentException(@"Cannot create image feature because texture is not readable", nameof(texture));
            // Save state
            this.type = new MLImageType(texture.width, texture.height, 4);
        ...
        public MLImageFeature RegionOfInterest (Rect rect, float rotation = 0f, Color32 background = default) => RegionOfInterest(
