...
namespace NatSuite.ML.Features {
    ...
    /// The array feature never owns or allocates its own memory.
    /// Instead, it provides a "view" into existing data.
    /// </summary>
    public sealed unsafe class MLArrayFeature<T> : MLFeature, IMLEdgeFeature, IMLCloudFeature where T : unmanaged {
        ...
        public MLArrayFeature (MLCloudFeature feature) : this(FeatureData(feature), feature.shape) { }
        #endregion
        ...
        private static T[] FeatureData (MLCloudFeature feature) {
            // Check
            if (Array.IndexOf(HubTypes, feature.type) < 0)
