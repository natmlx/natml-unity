...
namespace NatSuite.ML.Features {
    ...
    #pragma warning restore 0618
        ...
        /// As such the Edge feature MUST be explicitly released by the client.
        /// </summary>
        /// <param name="edgeFeature">Edge feature. This MUST be an array feature.</param>
        public unsafe MLArrayFeature (IntPtr edgeFeature) : this((T*)edgeFeature.FeatureData(), FeatureShape(edgeFeature)) { }

        /// <summary>
        /// Create an array feature from a Hub feature.
        /// </summary>
        /// <param name="hubFeature">Hub feature. This MUST be an array feature.</param>
        public MLArrayFeature (MLHubFeature hubFeature) : this(FeatureData(hubFeature), FeatureShape(hubFeature)) { }
        #endregion
        ...
        private static readonly string[] HubTypes = new [] {
            HubDataType.Float, HubDataType.Double,
            HubDataType.SByte, HubDataType.Short,
            HubDataType.Int, HubDataType.Long,
            HubDataType.Byte, HubDataType.UShort,
            HubDataType.UInt, HubDataType.ULong,
        };
        ...
        private static T[] FeatureData (MLHubFeature hubFeature) {
            // Check
            if (Array.IndexOf(HubTypes, hubFeature.type) < 0)
                throw new ArgumentException(@"Hub feature is not an array feature", nameof(hubFeature));
            // Deserialize
            var rawData = Convert.FromBase64String(hubFeature.data);
            var data = new T[rawData.Length / sizeof(T)];
            Buffer.BlockCopy(rawData, 0, data, 0, rawData.Length);
            return data;
        }

        private static int[] FeatureShape (IntPtr edgeFeature) {
            edgeFeature.FeatureType(out var type);
        ...
        private static int[] FeatureShape (MLHubFeature hubFeature) {
            return hubFeature.shape.Length == 0 ? new [] { 1 } : hubFeature.shape; // For scalars
        }
        #endregion
    }
}